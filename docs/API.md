...
> **Declarative-only paradigm**: The Application Framework exclusively uses the Nuva declarative programming model. Imperative patterns (`create_widget`, `register_handler`, `ActivityManager`, `EventDispatcher`) are prohibited. See [CODING_STANDARD.md](CODING_STANDARD.md) and [NUVA_LANG.md](NUVA_LANG.md).

### 6.1 UI Components (Declarative)

```rust
// component declaration — declarative UI composition
...
### 6.2 Window Management (Declarative)
...
### 6.3 Event Handling (Declarative)

```rust
// events are declared as part of the component model, not registered imperatively
...
### 9.4 NvScheduler AI Intelligent Scheduler API

```rust
// NvScheduler system calls
...
### 9.5 NvBalancer Heterogeneous Hardware Balancer API

```rust
// NvBalancer system calls
...
### 9.6 NvPowerMgr Power Optimization API

```rust
// NvPowerMgr system calls
