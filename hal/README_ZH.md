...
| gpu/ | GPU 抽象：Maleoon GPU、命令队列、中断处理(fence/GART故障/挂起/命令完成)、VRAM分配器(最佳适配+合并)、基于能力的访问控制 |
| npu/ | NPU 抽象：Da Vinci NPU HAL 桥接、ONNX 运行时、AI 调度器、推理预测器、设备管理、中断处理(推理完成/错误/模型加载/挂起)、可回收模型内存管理器 |
| quantum/ | 量子密码：PQC（Kyber KEM、Dilithium 签名）、QRNG 量子随机数、QKD 量子密钥分发 |
| power/ | 电源管理：PMIC(ops桥接到驱动)、挂起/恢复、跨架构 C-state（MWAIT/WFI/idle）、DVFS硬件接口、热管理(85°C节流/105°C关机) |
...
| riscv64/ | RISC-V 64 架构特定 HAL（CPU、MMU、中断控制器 PLIC、SBI、定时器） |
...
| `riscv64` | arch = riscv64 | 启用 RISC-V 64 HAL |
